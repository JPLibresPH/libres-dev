(function() {
  ...
  document.querySelectorAll('.isotope-layout').forEach(function(isotopeItem) {
   ...
   /**
   * Random filtering soft skills 
   */

   const skills = [
    { id: "skill-1", text: "Communication" },
  ...
  const options = {
    root: null,
    rootMargin: '0px',
    threshold: 0.3 // 60% visible before triggering
  };
  ...
  const observer = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const skill = skills.find(s => s.id === el.id);
        if (!el.classList.contains("animated")) {
          let i = 0;
          function type() {
            if (i <= skill.text.length) {
              el.textContent = skill.text.substring(0, i);
              i++;
              setTimeout(type, 60);
            }
          }
  ...
  // Observe all animated-skill spans
  document.querySelectorAll(".animated-skill").forEach(el => {
    observer.observe(el);
  });

  /**
   * For Porfolio Details move to web portfolio
   */
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (filter) {
      // Wait for Isotope and filters to be ready
      setTimeout(() => {
        const filterBtn = document.querySelector(`[data-filter=".filter-${filter}"]`);
        if (filterBtn) {
          filterBtn.click();
        }
  ...
  /**
   * For Percent Graph of Skills
   */

  document.addEventListener("DOMContentLoaded", () => {
    const charts = document.querySelectorAll("figure[class^='chart-']");
